    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {["HOME", "ABOUT US", "RESTAURANT", "GALLERY", "CONTACT"].map(
              (item, index) => (
                <a
                  key={index}
                  href={
      ...
      >
        <div className="bg-background px-6 pb-6 space-y-4 border-t border-border">
          ...
          <a onClick={() => setIsOpen(false)} href="/restaurant" className="block text-sm">RESTAURANT</a>
          <a onClick={() => setIsOpen(false)} href="/gallery" className="block text-sm">GALLERY</a>
